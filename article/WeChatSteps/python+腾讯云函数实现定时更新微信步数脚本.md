# 定时更新微信步数脚本(python+腾讯云函数)

## python代码

在刷新脚本后还通过邮件进行通知, 不进行通知的话, 相关信息可删除

![image-20230123193958032](../img/image-20230123193958032.png)![image-20230123194553168](../img/image-20230123194553168.png)
...
# ------------------------------
# 小米运动账号
xiaomi_user = value = os.environ.get('xiaomi_user')
# 小米运动密码
xiaomi_password = os.environ.get('xiaomi_password')
# 发信方的信息：发信邮箱，QQ 邮箱授权码
from_address = os.environ.get('from_address')
pop_code = os.environ.get('pop_code')
# 收信方邮箱
...
# 登录
def login(xiaomi_user, xiaomi_password):
    url1 = "https://api-user.huami.com/registrations/+86" + xiaomi_user + "/tokens"
    headers = {
...
# 主函数
def main():
    login_token, userid = login(xiaomi_user, xiaomi_password)
    if login_token == 0:
        print("登陆失败！")
        return "login fail!"
    ...
    step = str(randint(7000, 12000))  # 周一到五，指定范围内取随机数
    if week_index < 5:
        data_json = re.sub(findstep.findall(data_json)[0], step, str(data_json))
    else:
        step = str(randint(9000, 25000))  # 周六日随机取9000~25000步
        data_json = re.sub(findstep.findall(data_json)[0], step, str(data_json))
    ...
    print(response)
    result = f"当前用户：{xiaomi_user}</br> 修改步数： {step}  </br> 修改结果: {response['message']}  </br> 修改时间: {datetime.datetime.now()} "
    # 邮件通知运行结果
    send_email(result)
    print(result)
    return result
...
# 发送QQ邮件
def send_email(msgStr):
    # 邮箱正文内容，第一个参数为内容，第二个参数为格式(plain 为纯文本)，第三个参数为编码
    msg = MIMEText(msgStr, 'plain', 'utf-8')
    # 邮件头信息
    msg['From'] = Header('腾讯云函数')  # 发送者
    msg['To'] = Header('郑峻杰QQ邮箱')  # 接收者
    subject = '腾讯云函数每日刷微信步数脚本信息'
    msg['Subject'] = Header(subject, 'utf-8')  # 邮件主题

    try:
        smtpobj = smtplib.SMTP_SSL(smtp_server)
        # 建立连接--qq邮箱服务和端口号（可百度查询）
        smtpobj.connect(smtp_server, smtp_server_port)
        # 登录--发送者账号和口令
        smtpobj.login(from_address, pop_code)
        # 发送邮件
...
在腾讯云的云函数界面配置对应的**基础配置**, **代码**, **环境变量**, **触发器**

https://console.cloud.tencent.com/scf/list-create?rid=1&ns=default&createType=empty

![image-20230123161031751](../img/image-20230123161031751.png)

![image-20230123161112352](../img/image-20230123161112352.png)

**高级配置**里的**环境配置**需要配置**环境变量**
...
![image-20230123161702776](../img/image-20230123161702776.png)

![image-20230123162037888](../img/image-20230123162037888.png)

![image-20230123163301009](../img/image-20230123163301009.png)

## 补充

印象中是绑定乐心运动, 代码里的小米运动是以前注释就写错了, 属于祖传粪山了

由于不懂, 便删掉了原有的几种通知方式, 因为个人习惯在微信的腾讯邮箱里查看消息, 便在此基础上加了邮箱通知的功能

初次使用腾讯云, 没有看各种文档, 居然能直接配置成功, 有点窃喜, 也说明了腾讯云函数还是注重易用性的